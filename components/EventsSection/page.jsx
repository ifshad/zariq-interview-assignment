    ...
    <div id="upcoming-events" className="md:py-12">
      ...
      <div className="container grid grid-cols-1 md:grid-cols-3 gap-5 my-5">
        {upcomingEvents.map((event) => (
          <div key={event.id} className="w-80 h-96 grid grid-rows-6 shadow-xl rounded">
            <div className="row-span-4">{event.imageUrl}</div>
            <div className="row-span-2 grid grid-cols-7 p-3">
              <div className="col-span-2 text-secondary-color text-3xl font-semibold p-1 border-b-2 border-secondary-color">{event.date}</div>
